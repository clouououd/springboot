...
//    PATCH : 자원의 일부 수정, 수정할 필드만 전송(자동 주입이 아닌 부분만 수정하는 쿼리문에서 사용)
    ...
    public String ex01(){
        log.info("매개변수 없고 리턴은 문자열");
        // 연산
        return "문자열";
    }
    // 2번 매개변수 1개 있고 리턴은 문자열
    @GetMapping("/testPage2/{rno}")
    public String ex02(@PathVariable("rno") Long replyNumber){
        log.info("매개변수 1개 있고 리턴은 문자열");
        // 연산
        return "문자열";
    }
    ...
    public ReplyVO ex03(){
        log.info("매개변수 없고 리턴은 JSON Object");
        // 연산
        return replyService.read(3L);
    }
    // 4번 매개변수가 여러개 있고 리턴은 JSON Object
    @PostMapping(value = "/testPage4")
    public ReplyVO ex04(@RequestBody String reply){
        // 넘겨받는 json데이터는 vo/dto로 받아야한다. vo/dto의 필드가 아니면은 path로 받아야한다.
        log.info("매개변수가 여러개 있고 리턴은 JSON Object");
        log.info(reply);
        // 연산
        return replyService.read(3L);
    }
    // 5번 매개변수가 여러개 있고 리턴은 JSON Arry
    @PostMapping("/testPage5")
    public List<ReplyVO> ex05(@RequestBody String reply){
        log.info("매개변수가 여러개 있고 리턴은 JSON Arry");
        log.info(reply);
        // 연산
        return replyService.getList(new Criteria(1, 10), 12L);
    }
    ...
    // 3번 매개변수 여러개 있고 리턴은 JSON Object

    // 4번 매개변수 여러개 있고 리턴은 JSON Array
}
